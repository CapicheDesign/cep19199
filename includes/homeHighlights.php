<section data-index=2 id="highlights">
    <div id="highlights-panel-1" class="panel__home panel__third light-blue-bg">
        <div class="panel--content">
            <span class="large-text">$67.7m</span>
            <span class="mid-size-text">Revenue up 14% Increased volumes</span>
        </div>
        <a class="arrow-yellow-fwd" href="#"></a>
    </div>
    <div id="highlights-panel-2" class="panel__home panel__third">
        <div class="panel--content">
            <span class="mid-size-text">We're serious about safety</span>
        </div>
        <a class="arrow-yellow-fwd" href="#"></a>
    </div>
    <div id="highlights-panel-3" class="panel__home panel__third yellow-bg" >
        <div class="panel--content">
            <span class="mid-size-text">Strong trades</span>
            <div class="panel--content--inner">
                <div class="panel--content--half">
                    <div role="presentation" class="panel--icon__truck"></div>
                    <span class="large-text">1.7<small>m</small></span>
                    <span class="small-text">Record logs</span>
                </div>
                <div class="panel--content--half">
                    <div class="panel--content">
                        <div role="presentation" class="panel--icon__ship"></div>
                        <span class="large-text">110</span>
                        <span class="small-text">Record cruise</span>
                    </div>
                </div>
            </div>
        </div>
        <a class="arrow-white-fwd" href="#"></a>
    </div>
    <div id="highlights-panel-4" class="panel__home panel__third">
        <div class="panel--content">
            <span class="mid-size-text">Our People</span>
        </div>
        <a class="arrow-yellow-fwd" href="#"></a>
    </div>
    <div id="highlights-panel-5" class="panel__home panel__third white-bg">
        <div class="panel--content">
            <div role="presentation" class="panel--icon__worker"></div>
            <span class="mid-size-text">Regeneration strategy – medium-term plan in action</span>
        </div>
        <a class="arrow-yellow-fwd" href="#"></a>
    </div>
    <div id="highlights-panel-6" class="panel__home panel__third black-bg">
        <div class="panel--content">
            <div role="presentation" class="panel--icon__leaf"></div>
            <span class="mid-size-text">Commitment to sustainability and our environment</span>
        </div>
        <a class="arrow-yellow-fwd" href="#"></a>
    </div>
</section>